...
-->
  ...
  <body>
    ...
    <br>
    <?php echo form_open('carrera'); ?>
    <input type="hidden" id="bandPost" name="bandPost" value="1">
   

    <p align="center" class="Titulo">Control de Carreras</p>
    <table width="736px" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#E9E8E2">
      ...
      <tr>
        ...
        <select name="cmbCampo" id="cmbCampo" style="width:200px">
          <option></option>
           <option value="nombre_carrera" <?php if($campo=="nombre_carrera") echo 'selected';?> >Nombre</option>
          <option value="descripcion_carrera" <?php if($campo=="descripcion_carrera") echo 'selected';?> >Descripción</option>
        </select>
      ...
      <tr>
        <td colspan="<?php echo $colSpanFullLine; ?>" align="right">
          <?php
          if($permisos['i'])
            echo anchor('carrera/carreraForm/-1','<img alt="Agregar" src="'.base_url().'system/application/views/imagenes/agregar.png" border="0">');
          ?>        </td>
      </tr>
      ...
      ?>
          <tr>
            ...
            <td class="celdaContenido">
            <?php echo($row->descripcion_carrera); ?>            </td>
            <td align="center" class="celdaContenido">
            <?php
            if($permisos['m'])
              echo anchor('carrera/carreraForm/'.$row->carrera_id,
                        '<img alt="Modificar" src="'.base_url().'system/application/views/imagenes/modificar.png" border="0">');
            ?>            </td>
            <td align="center" class="celdaContenido">
            <?php
            if($permisos['b'])
              echo anchor('carrera/carreraDelete/'.$row->carrera_id,
