...
-->
  ...
  <body>
    ...
    <br>
    <?php echo form_open('beca'); ?>
    <input type="hidden" id="bandPost" name="bandPost" value="1">
   

    <p align="center" class="Titulo">Control de Becas</p>
    <table width="736px" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#E9E8E2">
      ...
      <tr>
        ...
        <select name="cmbCampo" id="cmbCampo" style="width:200px">
          <option></option>
           <option value="nombre_beca" <?php if($campo=="nombre_beca") echo 'selected';?> >Descripción</option>
          <option value="monto_beca" <?php if($campo=="monto_beca") echo 'selected';?> >Monto</option>
        </select>
      ...
      <tr>
        <td colspan="<?php echo $colSpanFullLine; ?>" align="right">
          <?php
          if($permisos['i'])
            echo anchor('beca/becaForm/-1','<img alt="Agregar" src="'.base_url().'system/application/views/imagenes/agregar.png" border="0">');
          ?>        </td>
      </tr>

      <tr>
        <td class="celdaTitulo" width="42%">
        Nombre de la Beca</td>
        <td class="celdaTitulo" width="10%" align="left">Monto (Bs.)</td>
        <td class="celdaTitulo" width="32%">&nbsp;</td>       
      ...
      ?>
          <tr>
            ...
            <td class="celdaContenido" align="right"><?php echo(number_format($row->monto_beca,2,',','.')); ?></td>
            <td class="celdaContenido" align="right">
              <div align="left"></div></td>
            <td align="center" class="celdaContenido">
            <?php
            if($permisos['m'])
              echo anchor('beca/becaForm/'.$row->beca_id,
                        '<img alt="Modificar" src="'.base_url().'system/application/views/imagenes/modificar.png" border="0">');
            ?>            </td>
            <td align="center" class="celdaContenido">
            <?php
            if($permisos['b'])
              echo anchor('beca/becaDelete/'.$row->beca_id,
